...
const px = (value: number, round?: 'round' | 'ceil' | 'floor' | 'none'): string => {
  if (!round) round = 'ceil';
  switch (round) {
    case 'round':
...
class Bounds {
  ...
  toString(): string {
    ...
    return `{ x: ${round(this.x)}, y: ${round(this.y)}, w: ${round(this.width)}, h: ${round(this.height)}, tw: ${round(
      this.totalWidth
    )}, th: ${round(this.totalHeight)} }`;
  }
}
