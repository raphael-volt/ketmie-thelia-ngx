import { Directive, ElementRef, Input, Output, OnChanges, SimpleChanges, SimpleChange, EventEmitter } from '@angular/core';
...
export class SliderDirective implements OnChanges {
  ...
  @Output()
  events: EventEmitter<SliderEvent> = new EventEmitter<SliderEvent>()
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if(change && isSliderDirection(change.currentValue)) {
      if(isSliderDirection(change.previousValue))
        removeClass(cl, sliderClass(change.previousValue))
      addClass(cl, sliderClass(change.currentValue))
    }
    change = changes.sliderState
    if (change) {
      if (!isSliderState(change.currentValue))
        this._sliderState = "none"
      else
        this._sliderState = change.currentValue
