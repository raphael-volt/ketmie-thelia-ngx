...
class Baseobjclassable extends Baseobj
{
    ...
    private function construire_where_clause($avec_and = true)
    {
        ...
        if (! $avec_and)
            $where_colonne = rtrim($where_colonne, " and ");
    ...
    public function prochain_classement()
    {
        ...
        if ($resul) {
            $classement = 1 + $this->get_result($resul);
        }
    ...
    public function changer_classement($id, $sens)
    {
        if ($this->charger_id($id)) {
            ...
            if ($sens == "M")
                ...
                return;
            $q = "select id, classement from " . $this->table . " where $where_colonne classement $req limit 0,1";
            $res = $this->query($q);
            if ($res) {
                $n = $this->num_rows($res);
                if($n) {
                    $p = $res->fetchObject(stdClass::class);
                    $repl_id = $p->id;
                    $repl_classement = $p->classement;
