 ...
 * **********************************************************************************
 */
function apiLog($data, $print = false)
{
    if ($print)
        $data = print_r($data, true);
    file_put_contents(__DIR__ . "/api.log", $data . PHP_EOL, FILE_APPEND);
}

/**
 * Permet de rediriger vers une page donnée, ou la page par défait en fin d'action...
 *
 * @param string $urlindiquee
 *            l'URL fournie, ou false pour ne pas rediriger.
 * @param string $urldefaut
 *            l'URL par defaut.
 */
function redirige_action($urlindiquee, $urldefaut)
{
    if ($urlindiquee !== '') {
        ...
        if ($urlindiquee !== false)
            redirige(str_replace("&amp;", "&", $urlindiquee));
    } else if ($urldefaut != "")
        redirige($urldefaut);
}

/*
 * function modifierdecli($arg0=null, $arg1=null, $arg2=null, $arg3=null)
 * {
 * dieItem(array($arg0, $arg1, $arg2, $arg3));
 * }
 */
// ajout panier
...
function __paiement($type_paiement)
{
...
{
    try {
        $fCarac = new Filtrecarac();
        $fCarac->processPaiement($type_paiement);
        /*
         * $filtreBo = new Filtretarifbo();
         * $filtreBo->calcule(array("paiement||$type_paiement||0", "paiement", "$type_paiement||0"));
         */
    } catch (Exception $e) {
        // FIXME: Echec de commande -> cas à traiter ?
    }
...
function creercompte($raison, $entreprise, $siret, $intracom, $prenom, $nom, $adresse1, $adresse2, $adresse3, $cpostal, $ville, $pays, $telfixe, $telport, $email1, $email2, $motdepasse1, $motdepasse2, $parrain, $obligetel = 0, $urlok = "", $urlerr = "")
{
    ...
    if ($client->raison != "" && $client->prenom != "" && $client->nom != "" && $client->email != "" && $client->motdepasse != "" && $client->email && ! $client->existe($email1) && $client->adresse1 != "" && $client->cpostal != "" && $client->ville != "" && $client->pays != "" && $obligeok) {
        ...
        $mailer = new TheliaNewAccoutMailer($client, $motdepasse1);
        
        $filename = __DIR__ . "/" . time() . "_mail.log";
...
function printInLog($object)
{
    addLog(print_r($object, true));
}

function addLog($str)
{
    file_put_contents(THELIA_DIR . "/client/myLog.txt", $str . PHP_EOL, FILE_APPEND);
}

function clearLog()
{
    file_put_contents(THELIA_DIR . "/client/myLog.txt", "");
}

function dieItem($item)
{
