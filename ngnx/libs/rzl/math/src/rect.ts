...
export class Rect {
  ...
  intersection(_r: Rect): Rect {
    const _a: Rect = this;
    if (this.contains(_r)) return _r;
    if (_r.right <= _a.x || _r.x >= _a.right || _r.bottom <= _a.y || _r.y >= _a.bottom) return null;
    const _result: Rect = new Rect();
    const _tl: Point = new Point();
    const _br: Point = new Point();

    if (_r.x < _a.x) _tl.x = _a.x;
    else _tl.x = _r.x;

    if (_r.right > _a.right) _br.x = _a.right;
    else _br.x = _r.right;

    if (_r.y < _a.y) _tl.y = _a.y;
    else _tl.y = _r.y;

    if (_r.bottom < _a.bottom) _br.y = _r.bottom;
    else _br.y = _a.bottom;
  ...
  contains(rect: Rect): boolean {
    ...
    return _rL >= _aL && _rT >= _aT && _rR <= _aR && _rB <= _aB;
  }
  get clone(): Rect {
...
const letterBox = (layout: Rect, child: Rect, maxScale: number = NaN): Rect => {
  ...
  if (sy < s) s = sy;
  if (!isNaN(maxScale) && s > maxScale) s = maxScale;
  result.scale(s, s);
  result.setPosition(layout.x + (layout.width - result.width) / 2, layout.y + (layout.height - result.height) / 2);
  return result;
};
