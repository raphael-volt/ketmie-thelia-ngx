...
ORDER BY classement";
		...
		*/
		self::$nearestProduits = array($prev->id, $next->id);
		return self::$nearestProduits;
	}
	...
	private function createAriane($separator="")
	{
		...
		while ($r->parent)
		{
			$l[] = array($r->label, $r->isLinkable() ? $this->getRubDescUrl($r):null);
			$r = $r->parent;
		}
		$l = array_reverse($l);
		if(ProdDesc::$current)
		{
			$l[] = array(ProdDesc::$current->label, $this->getProdDescUrl(ProdDesc::$current));
		}
		$n = count($l);
		for ($i = 0; $i < $n; $i++) 
		{
