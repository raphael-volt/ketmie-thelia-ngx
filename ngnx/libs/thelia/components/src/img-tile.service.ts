...
import { px } from '@ngnx/thelia/utils';
...
export class ImgTileService {
  ...
  updateTile(tile: TileData, layout: LayoutSizes) {
    ...
    let rects: RectHelper[] = tile.images.map(img => {
      if (img.naturalHeight < _maxHeight) _maxHeight = img.naturalHeight;
  ...
  _updateTile(tile: TileData, layout: LayoutSizes) {
    ...
    let row: { row: RectHelper; items: RectHelper[]; complete: boolean } = {
      row: new RectHelper(null, 0, 0, 0, _maxHeight),
      items: [],
      complete: false
    };
    ...
    for (item of items) {
      ...
      if (item == lastItem) row.complete = true;
    }
    if (!rows.length) {
