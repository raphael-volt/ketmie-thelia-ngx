...
import { LayoutService, getMaxScreenWidth } from '@ngnx/thelia/common';
import { XHRImageEvent } from '@ngnx/thelia/utils';
...
export class CategoryComponent extends SliderBaseComponent implements AfterViewInit, OnDestroy, OnChanges, OnInit {
  ...
  @ViewChild('imgCtn') imgCtnRef: ElementRef;
  imgCtn: HTMLElement;
  categoryId: string;
  ...
  private aliasChanged = alias => {
    ...
    // r = 0,182291667
    // r1280 = 0,2734375
    this.imgHeight = Math.ceil(0.15 * w);
  };
  ...
  constructor(
    ...
    this.subColl.add = layout.responsiveAliasChange.subscribe(this.aliasChanged);
  ...
  private events: XHRImageEvent[] = [];
  private tileId: string;
  percentLoaded: number = 0;
  imgLoad(event: XHRImageEvent) {
    ...
    for (event of this.events) {
      if (event.complete) loaded++;
    }
    this.percentLoaded = loaded / n * 100;
    // console.log(loaded, n)
    if (n == loaded) {
      this.loading = false;
      if (!this.imgCtn) this.imgCtn = this.imgCtnRef.nativeElement;
