...
import {
  ...
  getImgBoxMaxHeight,
  getMaxScreenWidth
} from '@ngnx/thelia/common';
import { RequestService } from '@ngnx/thelia/api';
import { SubscriptionCollector } from '@ngnx/thelia/utils';
import { ProductDetail } from '@ngnx/thelia/model';
import { SimpleMatrix, Point, Rect, interpolate } from '@ngnx/rzl/math';
import { draw, clearCanvas, px, getComputedBounds } from '@ngnx/thelia/utils';
import { TweenBase, TweenChangeEvent, Quadratic } from '@ngnx/rzl/tween';
...
export class ImgBoxDirective implements OnChanges, OnDestroy, AfterViewInit {
  ...
  private aliasChange = (alias?: ResponsiveAliases) => {
    const sl = Math.ceil(getMaxScreenWidth() * 0.5);
    if (sl != this._sideLength) {
      this._sideLength = sl;
      this._aliasChanged = true;
    }
    if (!this._initialized || !this.imgBox) return;
    // this.layoutChange()
    this.createUrls(this.imgBox.images);
    let i = 0;
    if (this._currentIndex > -1) i = this._currentIndex;
    this._subHelper.add = this.showImageAt(i, NEXT).subscribe(v => {}, null, () => {
  ...
  private _tween: TweenBase = new TweenBase({
    start: 0,
    end: 1,
    duration: 350,
    ease: Quadratic.out
  ...
  constructor(ref: ElementRef, request: RequestService, private layout: LayoutService) {
  ...
  prev() {
    ...
    if (next < 0) next = this._urls.length - 1;
  ...
  next() {
    if (!this._navEnabled) return;
    let next = this._currentIndex + 1;
    if (next >= this._urls.length) next = 0;
  ...
  private setProduct(p: ProductDetail) {
    ...
    this.createUrls(ids);
    this._subHelper.add = this.showImageAt(0, PREV).subscribe(v => {}, null, () => {
      this._subHelper.unsubscribeAll();
    });
  }
  ...
  private startAnimate(dir): Observable<boolean> {
    return Observable.create((observer: Observer<boolean>) => {
      ...
      let sub = tw.events.subscribe((e: TweenChangeEvent) => {
        ...
        for (let sd of [_data.current, _data.previous]) {
          ...
          currentRect.x = interpolate(e.value, sd.xFrom, sd.xTo);
  ...
  private getSlideData() {
    ...
    const gap = cw * 0.1;
    let imgs: Point[] = [null, null];
    if (curImg) imgs[1] = new Point(curImg.naturalWidth, curImg.naturalHeight);
    if (prevImg) imgs[0] = new Point(prevImg.naturalWidth, prevImg.naturalHeight);
    ...
    let cur: Rect = curImg ? new Rect(0, 0, scaledImgs[1].x, scaledImgs[1].y) : null;
    // prev is current image to hide  at index 0
    let prev: Rect = prevImg ? new Rect(0, 0, scaledImgs[0].x, scaledImgs[0].y) : null;
