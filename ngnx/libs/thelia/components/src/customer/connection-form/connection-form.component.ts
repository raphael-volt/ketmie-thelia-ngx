...
*/
    ...
    this.createForm = fb.group({
      email: new FormControl(this.data.email, (c: AbstractControl) => {
        ...
        if (!e) {
          ...
          this.checkTaken();
        } else this.errors.e1 = e ? 'email invalid.' : null;
        validateMails();
        return e;
      }),
  ...
  private checkTaken(updateMail: boolean = false) {
    ...
    if (this.emailTaken) {
      ...
      if (this.createForm.controls.email.valid) this.errors.e1 = null;
    }
  ...
  selectionChange(event: { selectedIndex: number }) {
    this.sendLabel = event.selectedIndex == 0 ? NEXT : SEND;
  }

  canNext() {
    if (this.isClient) return this.connectForm.valid;
    const stp: MatHorizontalStepper = this.stepper;
    if (!stp) return false;
    if (stp.selectedIndex == 0) return this.createForm.valid;
    return Boolean(this._validAddress);
  }
  ngOnInit() {}

  next() {
    if (this.isClient) return this.submit();
