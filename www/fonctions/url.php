...
function urlsite($idlang = false) {
	...
	if ($idlang === false)
		$lang = new Lang(ActionsLang::instance()->get_id_langue_courante());
	else
		$lang = new Lang($idlang);

	if (empty($lang->url) || (Variable::lire('un_domaine_par_langue') == 0))
		return  Variable::lire('urlsite');
	else
		return $lang->url;
}
...
function supprimer_panier($url) {

	// Supprimer l'éventuel paramètre de déconnexion, en, préservant les éventuels autres.
	$url = preg_replace("/\?panier=1\&(.+)$/", "?\\1", $url);
	$url = preg_replace("/(\?panier=1$)|((\&)*panier=1)/", "", $url);
...
function url_page_courante() {
	...
	if(strstr($uri, "/?fond") === 0)
	{
		return urlsite() . $uri;
	}

	// $_SESSION["a"] = $_SERVER['REQUEST_URI']; /?fond=ajax/print
