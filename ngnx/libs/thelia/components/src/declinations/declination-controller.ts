...
import { IDeclination, IDeclinationItem, CardItem, ProductDetail, Product } from '@ngnx/thelia/model';
...
export class DeclinationController implements OnChanges, OnInit, OnDestroy {
  ...
  set declination(value: IDeclination) {
    if (this._declination == value) return;
  ...
  @Output() declinationItemChange: EventEmitter<IDeclinationItem> = new EventEmitter<IDeclinationItem>();
  ...
  set declinationItem(value: IDeclinationItem) {
    if (this._declinationItem == value) return;
  ...
  set cardItem(value: CardItem) {
    if (this._cardItem == value) return;
    this._cardItem = value;

    this.cardItemChange.emit(value);
  }

  @Output() productChange: EventEmitter<ProductDetail> = new EventEmitter<ProductDetail>();
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (itemChange || productChange) {
      ...
      if (product && item) {
        if (product.id != item.product.id) throw new EvalError('product.id must equals cardItem.productId');
      }
      target = product || item;
    ...
    if (item) decliItem = decli.getItem(declinationId, item.decliId);
    else decliItem = null;
