...
import { Card, ICard, CardItem, CardItemPerso, Product, ProductDetail } from '@ngnx/thelia/model';
      ...
      providers: [
        ...
        },
        {
          ...
          useFactory: (h: Http, request: RequestService, session: SessionService) => {
            if (!api) api = new ApiService(h, session, request);
            return api;
          }
        },
        {
          ...
          useFactory: (_api: ApiService, request: RequestService, decli: DeclinationService) => {
            if (!service) service = new CardService(_api, request, decli);
            return service;
          }
        }
      ],
