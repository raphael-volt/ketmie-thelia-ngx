import { getComputedBounds } from './css-utils';
import { Rect } from '@ngnx/rzl/math';

const getContext = (
  canvas: HTMLCanvasElement,
  alpha: boolean = false,
  readFrequently: boolean = false
...
const drawToCanvas = (
  ...
  if (!ctx) ctx = getContext(canvas);
  clearCanvas(ctx, canvas);
  if (!src) return;
  const bounds = getComputedBounds(canvas);
  if (isNaN(bounds[0]) || isNaN(bounds[1])) throw new Error('Cant get canvas dimensions.');
  ...
  ctx.drawImage(src, 0, 0, size[0], size[1], 0, 0, bounds[0], bounds[1]);
};

const draw = (ctx: CanvasRenderingContext2D, src: HTMLImageElement | HTMLVideoElement, srcRect: Rect, dstRec: Rect) => {
  ctx.drawImage(
    src,
    srcRect.x,
    srcRect.y,
