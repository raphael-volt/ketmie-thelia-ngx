...
class Filtrecarac extends FiltreCustomBase
{
            ...
            case "panierPriceCell":
                {
                    ...
                    if ($perso) {
                        ...
                        foreach ($metals as $metal) {
                            ...
                            $carac = $this->getBoDeclinations($id, $metal);
                            try {
    ...
    private function getBOProdView($id_produit, $caracdisp)
    {
        ...
        $dict = $GLOBALS["dicotpl"];
        $caracs = $this->getBoDeclinations($caracdisp);
    ...
    private function addOptionNode(DOMDocument $doc, DOMElement $parent, BODecli $carac, $selected = false)
    {
     ...
     * @return DOMElement
     */
    private function addRadioNode(DOMDocument $doc, DOMElement $parent, BODecli $carac, $selected = false)
    {
        $li = $doc->createElement("li");
        $parent->appendChild($li);
        $li instanceof DOMElement;
        $li->setAttribute("class", "radioBox");
     ...
     * @return BODecli[]
     */
    private function getBoDeclinations($declidisp)
    {
...
ORDER BY cdd.titre, bo.size;";
        $stmt = $pdo->prepare($q);
        $stmt->setFetchMode(PDO::FETCH_CLASS, BODecli::class);
    ...
    private function getBoSelector(DOMDocument $doc, Perso $perso, $id_prod)
    {
        ...
        $node->setAttribute("name", "declinaison5");
        $caracs = $this->getBoDeclinations($carcdisp);
        foreach ($caracs as $carac) {
            $this->addOptionNode($doc, $node, $carac, $carac->id == $perso->valeur);
        }
        return $node;
    }
    ...
    private function getPanierTable($fond, $editable, $form = null)
    {
            ...
            if ($art->perso && count($art->perso)) {
                ...
                if ($perso->declinaison != BO_DECLINAISON)
                    ...
                    if ($editable) {
                        ...
                        $node->appendChild($this->getBoSelector($doc, $perso, $art->produit->id));
                    } else
                        $perso = null;
                }
            }
        ...
        $j = $panier->total();
        $ulQ->appendChild($this->createElement("li", $doc, $dict["totalpanier"] . " :"))
            ->setAttribute("class", "rightCell");
        $ulT->appendChild($this->createElement("li", $doc, $this->price_format($j) . " $euro"))
            ->setAttribute("class", "rightCell");
        if ($fond == "commande") {
            $port = port();
            $ulQ->appendChild($this->createElement("li", $doc, $dict["fraislivraison"] . " :"))
                ->setAttribute("class", "rightCell");
            $ulT->appendChild($this->createElement("li", $doc, $this->price_format($port) . " $euro"))
                ->setAttribute("class", "rightCell");
            $ulQ->appendChild($this->createElement("li", $doc, $dict["total"] . " :"))
                ->setAttribute("class", "rightCell total");
            $ulT->appendChild($this->createElement("li", $doc, $this->price_format($j + $port) . " $euro"))
                ->setAttribute("class", "rightCell total");
            $ulQ->appendChild($this->createElement("li", $doc, $dict["choixmodepaiement"] . " :"))
                ->setAttribute("class", "rightCell");
    ...
    private function getCommandeTable($commandeID)
    {
        ...
        $vps = $pdo->fetchAll($stmt, Venteprod::class, true);
        ...
        $ulQ->appendChild($this->createElement("li", $doc, $dict["fraislivraison"] . " :"))
            ->setAttribute("class", "rightCell");
        $ulT->appendChild($this->createElement("li", $doc, $this->price_format($commande->port) . " $euro"))
            ->setAttribute("class", "rightCell");
        
        $ulQ->appendChild($this->createElement("li", $doc, $dict["totalcommande"] . " :"))
            ->setAttribute("class", "rightCell total");
        $ulT->appendChild($this->createElement("li", $doc, $this->price_format($commande->total(true)) . " $euro"))
            ->setAttribute("class", "rightCell total");
