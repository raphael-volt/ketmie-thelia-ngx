...
export class DeclinationService {
  ...
  findDeclination(declinations: string[]): string {
    if (declinations && declinations.length) for (const p of declinations) if (this.has(p)) return p;
    return null;
  }
  ...
  getId(d: IDeclination): string {
    for (const i in declinationMap) if (declinationMap[i] == d) return i;
    return null;
  }
  get(id: string): IDeclination {
    if (this.has(id)) return declinationMap[id];
    return null;
  }

  getItem(id: string, itemId): IDeclinationItem {
    if (this.hasItem(id, itemId)) return declinationMap[id].items[itemId];
