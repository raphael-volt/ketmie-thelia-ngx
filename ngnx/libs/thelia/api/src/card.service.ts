...
export class CardService implements ICardImpl<Card, CardItem> {
  ...
  constructor(public api: ApiService, public request: RequestService, private declinationService: DeclinationService) {}
  ...
  get(next) {
    if (this._card) return next(this._card);
    this.getCard(next);
  }
  ...
  private getCardRequest() {
    ...
    return this.api.get(this.request.getRequest(this.request.getSearchParam('card', p)));
  }

  private getCardMap: any[];
  private getCard(next) {
    if (!this.getCardMap) this.getCardMap = [];
    const l = this.getCardMap;
    l.push(next);
    if (l.length == 1) {
      one(this.getCardRequest(), response => {
        ...
        this.notify('get');
        for (let n of this.getCardMap) n(this._card);
  ...
  private notify(action: CardAction, item?: CardItem) {
    this.events.emit(createCardEvent(action, this._card, item));
  }
  ...
  private getRequest(cardAction: CardAction, body?: CardItem) {
    return this.request.getRequest(this.request.getSearchParam('card', { cardAction: cardAction }), body);
  }
  ...
  private get checkCard(): void {
    if (!this.card) throw new Error('card is undefined');
    return;
  }

  private one(action: CardAction, item: CardItem, next: ICNext<APIResponse>) {
    one(this.api.post(this.getRequest(action, item)), next);
  }
}
