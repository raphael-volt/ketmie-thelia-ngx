...
EOT;
		...
		$uid = intval($pdo->query($q)->fetch(PDO::FETCH_COLUMN, 0));
		//$uid ++;
		$autoTitre = "$uid";
		...
		$query = "
SELECT d.titre as t, parent AS p, lien AS l FROM rubrique as r
...
WHERE r.id=?";
		...
		while($rid != 0)
		{
			$pdo->bindInt($stmt, 1, $rid, true);
			$p = $stmt->fetch(PDO::FETCH_OBJ);
			if($rid == $rubrique)
				$autoTitre = "$p->t $autoTitre";
			if($p->l == "shop")
				break;
			...
			array_unshift($refPref, strtoupper($p->l));
			$rid = intval($p->p);
		}
		if($refPref && count($refPref))
		{
			$autoRef = implode("-", $refPref) . "-" . $uid;
		}
		else 
		{
			$autoRef = "REF-" . $uid;
		...
		$titre = $autoTitre;
		// die("<PRE>" . print_r(array($autoRef, $titre), true));
	}
...
<?php
    switch($action){
        ...
        case 'modifier' : modifier($id, $lang, $ref, $prix, $ecotaxe, $promo, $prix2, $rubrique, $nouveaute, $perso, $poids, $stock, $tva, $ligne, $titre, $chapo, $description, $postscriptum, $urlsuiv, $urlreecrite); break;
        case 'ajouter' : 
        {
...
<?php
    ...
    function ajouter($lang, $ref, $prix, $ecotaxe, $promo, $prix2, $rubrique, $nouveaute, $perso, $poids, $stock, $tva, $ligne, $titre, $chapo, $description, $postscriptum){
        ...
        // $q = "SELECT cp.price FROM rubcaracteristique as rc JOIN carac_price as cp ON cp.caracteristique=rc.caracteristique WHERE rc.rubrique=?";
        $q = "SELECT caracteristique FROM rubcaracteristique WHERE rubrique=?";
        // die("rubrique:" . $rubrique);
        $pdo = PDOThelia::getInstance();
		$stmt = $pdo->prepare($q);
		$pdo->bindInt($stmt, 1, $rubrique, true);
		$res = $pdo->fetchColumn($stmt, 0);
		if($res !== null)
		{
			$res = intval($res);
			switch ($res) 
			{
				...
				{
					$prix = Variable::lire(COLLAGE_PRIX_VARIABLE);
				}
				break;
    	...
    	// die("<pre>" . print_r(array("ref"=>$ref, "titre"=>$titre, "prix"=>$prix), true));
        ...
        $pdo = PDOThelia::getInstance();
        $rubcaracteristique = new Rubcaracteristique();
        $query = "SELECT caracteristique FROM $rubcaracteristique->table WHERE rubrique={$rubrique}";
        $result = $pdo->query($query);
        $result = $result->fetchAll(PDO::FETCH_COLUMN, 0);
        $query = "SELECT cdd.caracdisp FROM caracdisp as cd
LEFT JOIN caracdispdesc as cdd ON cdd.caracdisp=cd.id
WHERE cdd.classement=1 AND cd.caracteristique=?";
        $stmt = $pdo->prepare($query);
        	
        $defaults = array();
        foreach ($result as $caracteristique)
        {
        	$pdo->bindInt($stmt, 1, $caracteristique, true);
        	$caracdisp = $pdo->fetchColumn($stmt, 0);
        	$caracval = new Caracval();
...
<?php require_once("js/accessoire.php"); ?>
<?php require_once("js/contenu_associe.php"); ?>
...
<!-- bloc descriptif du produit -->
    ...
    <tr class="claire">
		...
		if($ref){
			...
			?>
			<th style="padding-top:10px;"> <input type="text" name="ref" id="ref_c" class="form_reference" onblur="verifref();" value="<?php echo($autoRef); ?>" /> <span id="verification_ref_c"> </span></th>
			<?php
		}
		?>
	</tr>
        ...
        <td style="vertical-align:top;"><select name="rubrique" id="rubrique" class="form_long">
        	<option value="0"><?php echo trad('A la racine', 'admin'); ?></option>
          	<?php if($ref)
          		...
          		echo arbreOption(0, 1, $rubrique, 0);
        	} ?>
        </select></td>
   	</tr>
    </table>
...
<!-- bloc des caractéristiques de base du produit -->
	...
	<table width="100%" cellpadding="5" cellspacing="0">
   	...
   	<tr class="claire">
        ...
        <th class="designation" width="133" ><?php echo trad('Nouveaute', 'admin'); ?></th>
   	...
   	<tr class="fonce">
        ...
        <td class="designation"><?php echo trad('En_promotion', 'admin'); ?></td>
        <td><input name="promo" id="promo" type="checkbox" class="form" <?php if($produit->promo) echo "checked=\"checked\""; ?> /></td>
   	</tr>
   		<tr class="claire">
        <td class="designation"><?php echo trad('TVA', 'admin'); ?></td>
        ...
        <td class="designation"><?php echo trad('Stock', 'admin'); ?></td>
  		...
  		if(! $nbres) { ?>
				...
				<ul class="ligne1">
						...
						<li class="cellule">
        					...
        					<input type="text" class="form_caracterisques_ajoutees" name="caract<?php echo($row->caracteristique); ?>" id="caract<?php echo($row->caracteristique); ?>" value="<?php echo(htmlspecialchars($caracval->valeur)); ?>" /></li>
        		</ul>
...
<div class="blocs_pliants_prod" id="pliantcontenusassocies">
		...
		<?php
				lister_contenuassoc(1, $produit->ref);
         ?>
		</ul>
...
<?php
	ActionsAdminModules::instance()->inclure_module_admin("produitmodifier");
?>
</div>
...
<div class="bloc_transfert">
	<div class="claire">
		<div class="champs" style="padding-top:10px; width:375px;">
			...
			?>
			<a title="Voir le produit en ligne" href="<?php echo urlfond("produit", "ref=$ref&amp;id_rubrique=$rubrique", true); ?>" target="_blank" ><img src="gfx/site.png" alt="Voir le produit en ligne" title="Voir le produit en ligne" /></a>
