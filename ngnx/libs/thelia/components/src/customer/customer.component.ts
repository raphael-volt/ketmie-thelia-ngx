...
import { SEND, OK, CREATE, CANCEL, PopupService, SnackBarService } from '@ngnx/thelia/common';
...
export class CustomerComponent extends SliderBaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private validateEmail: ValidatorFn = (c: AbstractControl): ValidationErrors | null => {
    const value: string = c.value;
    if (value == null || !value.length) return null;
    if (this.data.email == this.customer.email) return { notChanged: true };
  ...
  getEmailError(control: FormControl) {
    ...
    if (control.errors.notChanged === true) return `L'email n'est pas modifié.`;
  ...
  changeEmail() {
    let sub = this.customerService.changeEmail(this.data.email).subscribe(success => {
      this.canChangeEmail = false;
      if (success) {
        this.data.email2 = '';
  ...
  changePassword() {
    let sub = this.customerService.changePassword(this.data.password).subscribe(success => {
      this.data.password = '';
      this.data.password2 = '';
      this.pwdGroup.reset();
      if (!success) {
        this.modalError('La modification de votre mot de passe a échouée.');
      } else this.pwdGroup.markAsUntouched();
  ...
  ngAfterViewInit() {
    ...
    if (this.customer.isNew) {
      this.openSnackBar(
        `<i class="fa fa-smile-o" aria-hidden="true"></i> Bienvenue ${raisonsMap[this.customer.raison].long} ${
  ...
  saveCustomer() {
    let sub = this.customerService.updateCustomer(this.address).subscribe(success => {
      if (success) {
  ...
  logout() {
    let sub = this.customerService.logout().subscribe(response => {
  ...
  deleteDeliveryAddress(address: Address) {
    ...
    let sub = this.customerService.deleteAddress(address).subscribe(success => {
      sub.unsubscribe();
      if (success) {
        this.deliveryAddresses.splice(i, 1);
        this.openSnackBar("L'adresse a été supprimée.");
      } else this.modalError("La suppression de l'adresse a échouée.");
    });
  }
  ...
  editDeliveryAddress(address: Address) {
    ...
    let sub = ref.afterClosed().subscribe(address => {
      sub.unsubscribe();
      if (address) {
        sub = this.customerService.updateAddress(address).subscribe(success => {
          if (success) {
      ...
      } else this.openSnackBar("La modification de l'adresse a été annulée.");
    });
  }
  ...
  createDeliveryAddress() {
    ...
    let sub = ref.afterClosed().subscribe(address => {
      sub.unsubscribe();
      if (address) {
        sub = this.customerService.createAddress(address).subscribe(success => {
          if (success) {
      ...
      } else this.openSnackBar("La création de l'adresse a été annulée.");
    });
  }
}
...
export class BlurChildDirective implements OnDestroy {
  @Output() blurOut: EventEmitter<any> = new EventEmitter();
  ...
  private checkContais = (e: Event) => {
    if (!this.target.contains(e.target as Node)) this.blurOut.emit(true);
  };
