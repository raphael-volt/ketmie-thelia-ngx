...
class Panier {
	...
	function ajouter($ref, $quantite, $tdeclidisp=array(), $append=0, $nouveau=0, $parent=-1){
		...
		if ($this->verfistock($ref, $quantite, $tdeclidisp)) 
		{
			...
			if (! $existe || $nouveau == 1) {
				...
				$article = new Article($ref, $quantite, $tdeclidisp, $parent);
				if(count($article->perso))
				{
...
WHERE p.id=?";
	...
	private function getPdo()
	{
		if(! self::$pdo)
		{
			self::$pdo = PDOThelia::getInstance();
		}
		return self::$pdo;
	}
	...
	function modifier($article, $quantite, $parent=-1) {
		...
		if($parent>-2 && isset($this->tabarticle[$parent])) {
			$art->parent = $parent;
		}
	}
