...
const _ease = (t: number, d: TweenData) => d.ease(t, d.start, d.end - d.start, d.duration);
...
const _tick = () => {
  _requestFlag = false;
  _now = getNow();
  for (const t of _tweens) t.update(_now);
...
export class TweenBase {
  ...
  toggle() {
    ...
    if (pauseFlag) _addRequestAnimation(this);
    const n = getNow();
    this._reverseTime = n;
  ...
  setRunning(running: boolean, status: TweenStatus, pauseFlag: boolean = false, startTime: number = NaN) {
    ...
    if (!isNaN(startTime)) this._startTime = startTime;
  }
